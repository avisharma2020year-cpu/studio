      ...
      <div className="flex min-h-screen">
        <Sidebar collapsible="icon" className="border-r">
          <SidebarHeader className="p-4 border-b">
             {/* In collapsed state, AppLogo won't show text correctly. Maybe simplified logo here. */}
            <div className="group-data-[collapsible=icon]:hidden">
                <AppLogo size="md" />
            </div>
             <div className="hidden group-data-[collapsible=icon]:block text-center">
                <AppLogo size="sm" /> {/* Or just an icon */}
            </div>
          </SidebarHeader>
          ...
          <SidebarFooter className="p-2 border-t">
            {/* Optional: Add user avatar or settings shortcut for collapsed view */}
          </SidebarFooter>
        </Sidebar>
        ...
        <SidebarInset className="flex-1 flex flex-col bg-muted/30">
          <header className="sticky top-0 z-40 flex h-16 items-center justify-between gap-4 border-b bg-background px-6">
            <div className="flex items-center">
              ...
              <h1 className="text-lg font-semibold md:text-xl font-headline">Admin Dashboard</h1>
            </div>
            <div className="flex items-center gap-4">
              ...
              <AppHeader currentRole="admin" userName={currentUser.name} />
            </div>
          </header>
